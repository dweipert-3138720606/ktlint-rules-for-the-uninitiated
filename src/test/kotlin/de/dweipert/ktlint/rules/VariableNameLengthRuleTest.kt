...
class VariableNameLengthRuleTest {
    private val assertThat =
        KtLintAssertThat
            ...
            .withEditorConfigOverride(
                Pair(
                    RuleId("uninitiated:variable-name-length").createRuleExecutionEditorConfigProperty(RuleExecution.enabled),
    ...
    fun `should respect custom min length`() {
        assertThat("fun f() { val abcd = 1 }")
            .withEditorConfigOverride(Pair(MIN_VARIABLE_NAME_LENGTH_PROPERTY, 5))
    ...
    fun `should accept custom skip names`() {
        assertThat("fun f(x: Int) { val ab = 1 }")
            .withEditorConfigOverride(Pair(SKIP_VARIABLE_NAMES_PROPERTY, "_, it, x"))
    ...
    fun `should skip custom names`() {
        assertThat("fun f() { val ab = 1 }")
            .withEditorConfigOverride(Pair(SKIP_VARIABLE_NAMES_PROPERTY, "ab"))
            .hasNoLintViolations()
    }
    ...
    fun `should flag short variable with empty skip list`() {
        assertThat("fun f() { val _ = 1 }")
            .withEditorConfigOverride(Pair(SKIP_VARIABLE_NAMES_PROPERTY, ""))
